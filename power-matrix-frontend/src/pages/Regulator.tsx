...
import { fetchProposals, approveProposal } from "@/blockchain/funding";
...
const RegulatorPage: React.FC = () => {
  const { provider, account, connect } = useWallet(1n);
              ...
              <section>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
                      ...
                      >
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div className="text-xs text-[#B0B8D1]">
                            Goal:{" "}
                            <b className="text-[#0DF6A9]">
                              {Number(p.goalEth).toFixed(4)} ETH
                            </b>{" "}
                            · Raised:{" "}
                            <b className="text-[#FFD600]">
                              {" "}
                              {Number(p.raisedEth).toFixed(4)} ETH
                            </b>{" "}
                            · Deadline: <b>{timeLeft(p.deadline)}</b>
                          </div>

                          {/* Approve: auto-create tokenId + credits pool */}
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  ...
                                  if (!credits) {
                                    setStatus("Enter credits");
                                    return;
                                  }
                                  try {
                                    setStatus("Creating project token…");
                                    const signer = await provider.getSigner();
                                    const carbon = new Contract(
                                      CARBON_ADDR,
                                      (carbonAbi as any).abi,
                                      signer
                                    );

                                    // Defaults for the new credit series
                                    const vintage = new Date().getFullYear();
                                    const cap = BigInt(credits); // cap equals pool
                                    const uri = `${p.title} | auto:${p.id}`;

                                    // 1) Create token on CarbonCredit1155
                                    const tx1 = await (carbon as any).createProject(
                                      vintage,
                                      cap,
                                      uri
                                    );
                                    ...
                                    if (!tokenId) {
                                      setStatus("Could not get tokenId");
                                      return;
                                    }

                                    // 2) Approve proposal with tokenId + credits pool
                                    setStatus(
                                      `Approving with tokenId ${tokenId.toString()}…`
                                    );
                                    ...
                                    setStatus(
                                      e?.shortMessage || e?.message || "Approve failed"
                                    );
                                  }
                                }}
                              >
                            ...
                            <div className="text-xs text-[#B0B8D1]">
                              We’ll create a new carbon credit token automatically
                              and assign this credits pool.
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                  ...
                  {!pendingProjects.length && (
                    <div className="text-[#B0B8D1] sm:col-span-2 lg:col-span-3">
                      No pending projects.
                    </div>
                  )}
                </div>
          ...
          {tab === "claims" && (
            <div className="grid md:grid-cols-2 gap-6">
              {pendingClaims.map((c) => (
                <Card
                  key={c.id}
                  className="bg-[#181F36] rounded-2xl shadow-md border-0"
                >
                  <CardHeader>
                    <CardTitle className="text-lg">
                      Claim #{c.id} · tokenId {c.tokenId || "(new)"} · req{" "}
                      {c.requested}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="text-sm text-[#B0B8D1]">
                      Requested: <b>{c.requested}</b> credit(s) ·{" "}
                      <span className="opacity-80">
                        Approval mints requested amount
                      </span>
                    </div>

                    {/* Approve (fixed to requested amount) */}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            if (!req) {
                              setStatus("Requested amount is zero");
                              return;
                            }
                            setStatus("Approving…");
                            try {
                    ...
                    <div className="border-t border-[#232B45] pt-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Textarea
                          ...
                          value={rej[c.id] || ""}
                          onChange={(e) =>
                            setRej((s) => ({ ...s, [c.id]: e.target.value }))
                          }
                          className="bg-[#10182A] border-[#232B45] text-white flex-1"
                        />
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              await rejectClaim(
                                c.id,
                                rej[c.id] || "insufficient evidence"
                              );
                              setStatus("Rejected");
                              await loadAll();
                            } catch (e: any) {
                              setStatus(
                                e?.shortMessage || e?.message || "Reject failed"
                              );
                            }
                          }}
                        >
              ...
              {!pendingClaims.length && !loading && (
                <div className="text-[#B0B8D1] md:col-span-2">
                  No pending claims 🎉
                </div>
              )}
            </div>

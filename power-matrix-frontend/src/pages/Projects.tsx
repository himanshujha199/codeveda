...
interface Project {
  ...
  description: string;
  image: string;
  goal: number; // PMGT needed
  raised: number; // PMGT received
}
      ...
      <div className="bg-[#10182A] min-h-screen text-white font-inter p-8 mt-16">
        <div className="max-w-7xl mx-auto">
                ...
                <Card key={proj.id} className="bg-[#181F36] rounded-2xl shadow-md border-0 overflow-hidden">
                  ...
                  <CardContent>
                    <div className="text-sm mb-2 text-[#B0B8D1]">
                      ...
                      <span className="text-[#0DF6A9] font-semibold">{proj.goal}</span> PMGT
                    </div>
                    <Progress value={pct} />
                  </CardContent>
                  <CardFooter className="flex items-center justify-between">
                    <Button className="bg-[#0DF6A9] text-[#10182A] hover:bg-[#0DF6A9]/90">
                      Donate
                    </Button>
          ...
          <p className="text-center text-xs text-[#B0B8D1] mt-10">
            Donations are credited to project listers after verification. Donors receive 20% of their contribution back in PMGT.
          </p>
        </div>
      </div>
    </>
